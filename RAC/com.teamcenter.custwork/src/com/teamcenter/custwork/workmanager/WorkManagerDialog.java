...
public class WorkManagerDialog extends AbstractAIFDialog implements
		IWorkManagerTookit, TableModelListener {
	...
	public WorkManagerDialog(Frame frame, boolean flag) throws TCException {
		super(frame, flag);

		table_column_names = new String[] { "No", "Program", "Task Type", "N",
				"O", "W", "H" };
	...
	public void initUI() throws TCException {
		...
		ImageIcon image = CacheImage.getINSTANCE().convertibaleImageIcon(
				"ProcessVariable_32.png");
	...
	private JPanel getTitlePanel() throws TCException {
		...
		palTitle.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(),
				"Information"));
		palTitle.setBackground(Color.WHITE);

		reloadBtt_ = BuildComponentFactory.makeNavigationButton("reload.png",
				"Load", "Load");
		...
		nfPal_ = new JButton(new String(
				Integer.toString(totalNormalHoursAllowed_)));
	...
	private JToolBar getToolBar() throws TCException {
		...
		pal.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(),
				"Bill Type"));

		assignBtt_ = BuildComponentFactory.makeNavigationButton(null,
				" Assign ", " Assign ");
		assignBtt_.setBackground(Color.WHITE);
		assignBtt_.setBorderPainted(true);
		assignBtt_.setBorder(BorderFactory.createBevelBorder(
				BevelBorder.RAISED, Color.LIGHT_GRAY, Color.LIGHT_GRAY,
		...
		JButton clsBtt = BuildComponentFactory.makeNavigationButton(null,
				"  Clear  ", "  Clear  ");
		clsBtt.setBackground(Color.WHITE);
		clsBtt.setBorderPainted(true);
		clsBtt.setBorder(BorderFactory.createBevelBorder(BevelBorder.RAISED,
				Color.LIGHT_GRAY, Color.LIGHT_GRAY, Color.GRAY, Color.GRAY));
		...
		JButton checkStatusBtt = BuildComponentFactory.makeNavigationButton(
				null, "Status", "Status");
		...
		dciBtt_ = BuildComponentFactory.makeNavigationButton(null,
				"Del CostInfo", "Del CostInfo");
		...
		dciBtt_.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					WorkmanagerOperation.delCostInfo(session,
							"Delete_CostInfo", "Delete");
					dciBtt_.setBackground(ManHourObject.toolButtColor);
				} catch (Exception ee) {
					ee.printStackTrace();
				}
		...
		JButton idataBtt = BuildComponentFactory.makeNavigationButton(null,
				"I-Data", "I-Data");
		...
		JButton odataBtt = BuildComponentFactory.makeNavigationButton(null,
				"O-Data", "O-Data");
		...
		JButton mhinfoBtt = BuildComponentFactory.makeNavigationButton(null,
				"Mhinfo", "Mhinfo");
		...
		buttPanel.setBorder(new TitledBorder(
				...
				if (i > 0) {
					if (!canBeRemoved()) {
						JOptionPane
								.showConfirmDialog(
										null,
										"Saved man hours can't be deleted, please recall it first !",
										"Delete Confirmation",
		...
		if (session.getUserName().equals("infodba")
				|| session.getUserName().equals("axiaoyz")) {
			toolBar.add(dciBtt_);
		}
	...
	private JPanel getBottomPanel() throws TCException {
		...
		saveAllBtt_ = BuildComponentFactory.makeNavigationButton("wsave.png",
				"Save All", "Save All");
		...
		saveAllBtt_.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (!canAllBeSaved())
					return;
				try {
					new Thread(new SaveProgressDialog(WorkManagerDialog.this,
							"save all working hours", session, null,
							SaveProgressDialog.ACT_SAVEALL)).start();
		...
		reviseBtt_ = BuildComponentFactory.makeNavigationButton(null, "Recall",
				"Recall");
		...
		reviseBtt_.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					new Thread(new SaveProgressDialog(WorkManagerDialog.this,
							"Revise working hours", session, null,
							SaveProgressDialog.ACT_REVISE)).start();
				} catch (TCException e1) {
					e1.printStackTrace();
				}
			}
		...
		saveBtt_ = BuildComponentFactory.makeNavigationButton(null, "Save",
				"Save");
		...
		saveBtt_.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					new Thread(new SaveProgressDialog(WorkManagerDialog.this,
							"Save working hours", session, null,
							SaveProgressDialog.ACT_SAVE)).start();
				} catch (TCException e1) {
					e1.printStackTrace();
				}
			}
	...
	public void enReloadBtt() {
		...
		if (mhInfo_.myYear.equals(year.toString())
				&& mhInfo_.myMonth.equals(month.toString())) {
			reloadBtt_.setEnabled(false);
		} else {
		...
		try {
			if (saveAllBtt_.isEnabled()) {
				int n = JOptionPane.showConfirmDialog(null,
						"Save current data?", "Save Confirmation",
			...
			String year = this.datePanel.getYearComb().getTextField().getText();
			String month = this.datePanel.getMonthComb().getTextField()
					.getText();

			if (!mhInfo_.myYear.equals(year.toString())
					|| !mhInfo_.myMonth.equals(month.toString())) {
				mhInfo_ = null;
				mhInfo_ = WorkmanagerOperation.loadManHourInfo(session, year,
						month);
				initData();

				new Thread(new QueryProgressDialog(WorkManagerDialog.this,
						"loading working hours", session, null)).start();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void setFixedColumnTable(String ymName, String[] days,
			String[] weeks) {
		newCloumn = new String[table_column_names.length + weeks.length];
		System.arraycopy(table_column_names, 0, newCloumn, 0,
				table_column_names.length);
		...
		for (int i = 0; i < leftClomCount; i++) {
			...
			if (i == 2) {
				WComboBoxEditor tskEditor = new WComboBoxEditor(
						getTskComboBox(), true);
				tskEditor.setWorkManager(this);
				clom.setCellEditor(tskEditor);
			} else if (i == 1) {
				WComboBoxEditor prgEditor = new WComboBoxEditor(
						getPrgComboBox(), false);
		...
		for (int i = 0; i < rightClomCount; i++) {
			...
			if (!mhInfo_.theMonthTmp[i].isHoliday
				...
				rCellRenderer
						.setManHourBgColor(ManHourObject.noWorkingDayBackColor);
			}
			...
			public void itemStateChanged(ItemEvent e) {
				if (e.getStateChange() == ItemEvent.SELECTED) {
					...
					if (isDuplicatedProgramAndTaskType(row, col, tskStr)) {
						...
						JOptionPane.showConfirmDialog(null, mes, "Warning...",
								JOptionPane.DEFAULT_OPTION);
						tskCbox_.setSelectedIndex(-1);
					}
	...
	private boolean isDuplicatedProgramAndTaskType(int row, int col,
			String task_type) {
		...
		if (row > 0) {
			...
			for (int i = 0; i < row; i++) {
				...
				countValue = countValue
						+ Integer.parseInt(data != null
								&& data.toString().trim().length() > 0 ? data
								.toString() : "0");
			}
		...
		try {
			...
			for (i = 0; i < objData.length - 1; i++) {
				...
				for (int j = 7; j < objData[i].length; j++) {
					...
					List<ManHourObject> mheObjList = ManHourObject
							.getMheObjectList(mhObj0);
					...
					for (int pdx = 0; pdx < size; pdx++) {
						...
						if ((workHours == null || workHours.length() == 0 || workHours
								.equals("0"))) {
							continue;
						}
						...
						ManHourEntry mhe = createManHourEntry(prObjectName,
								taskTypeDval, billType, btInternal, brRef,
								workHours, status, tseref, mheref, i, j - 7);
						mheList.add(mhe);
					}
				}
			}
			...
			if (mheList != null && mheList.size() > 0) {
				...
				ManHourEntry[] mheObjs = (ManHourEntry[]) mheList
						.toArray(new ManHourEntry[mheListSize]);

				ManHourEntrySet mheSet = WorkmanagerOperation.save(session,
						mhInfo_, mheObjs);
	...
	private boolean isProgramClosed() {
		...
		for (int idx = 0; idx < selectedRows.length; idx++) {
			String ptStr = leftTable.getValueAt(selectedRows[idx], 1)
					.toString()
					+ ":"
	...
	private boolean findNonEditingCell() {
		...
		for (int i = 0; i < selectedRows.length; i++) {
			for (int j = 0; j < selectedColumns.length; j++) {
				...
				List<ManHourObject> mheList = ManHourObject
						.getMheObjectList(mhObj0);
				...
				for (int pdx = 0; pdx < size; pdx++) {
					...
					if (!(status.equals("Working") && tseRefStr.equals("Null") && mheRefStr
							.equals("Null"))) {
						return true;
					}
				}
	...
	private boolean canBeSaved() {
		...
		if (rightTable.getSelectedRows().length <= 0
			...
			JOptionPane.showConfirmDialog(null, message1, "Warning...",
					JOptionPane.DEFAULT_OPTION);
			return canSave;
		}
		...
		if (isAllSelectedCellEmpty()) {
			...
			JOptionPane.showConfirmDialog(null, message2, "Warning...",
					JOptionPane.DEFAULT_OPTION);
			return canSave;
		}
		...
		if (findNonEditingCell()) {
			...
			JOptionPane.showConfirmDialog(null, message3, "Warning...",
					JOptionPane.DEFAULT_OPTION);
			return canSave;
		}
		...
		try {
			...
			for (i = 0; i < selectedRows.length; i++) {
				...
				for (int j = 0; j < selectedColumns.length; j++) {
					...
					ManHourObject mhObj0 = (ManHourObject) datObj;
					List<ManHourObject> objList = ManHourObject
							.getMheObjectList(mhObj0);
					int size = (objList != null) ? objList.size() : 0;
					for (int pdx = 0; pdx < size; pdx++) {
						ManHourObject mhObj = objList.get(pdx);
						String workHours = mhObj.getWorkingHours();
						String billType = mhObj.getBillType() == null ? ""
								: mhObj.getBillType();
						String status = mhObj.getTseStatus();
						String tseref = mhObj.getTseRef();
						String mheref = mhObj.getMheRef();
						...
						ManHourEntry mhe = createManHourEntry(prObjectName,
								taskTypeDval, billType, btInternal, brRef,
								workHours, status, tseref, mheref,
								selectedRows[i], selectedColumns[j]);
						mheList.add(mhe);
					}
				}
			}
	...
	private boolean canBeRevised() {
		...
		if (findEditingCell()) {
			...
			JOptionPane.showConfirmDialog(null, message4, "Warning...",
					JOptionPane.DEFAULT_OPTION);
			return canRevise;
		}
		...
		if (isProgramClosed()) {
			...
			JOptionPane.showConfirmDialog(null, message5, "Warning...",
					JOptionPane.DEFAULT_OPTION);
			return canRevise;
		}
		...
		if (needAdminToHandle()) {
			...
			JOptionPane.showConfirmDialog(null, message6, "Warning...",
					JOptionPane.DEFAULT_OPTION);
			return canRevise;
		}
	...
	public void revise(SaveProgressDialog progressDialog) {
		...
		try {
			...
			if (mheList != null && mheList.size() > 0) {
				...
				ManHourEntrySet mheSet = WorkmanagerOperation.revise(session,
						mhInfo_, mheObjs);
	...
	private String printMHEObject(ManHourObject mhe_obj) {
		...
		if (mhe_obj == null) {
			...
			mheStr = "[ " + mhe_obj.getWorkingHours() + "  "
					+ mhe_obj.getBillType() + "  " + mhe_obj.getTseStatus()
					+ "  " + mhe_obj.getTseRef() + "  " + mhe_obj.getMheRef()
					+ " ]";
		}
		return mheStr;
	}
	...
	private String printMHEList(List<ManHourEntry> mhe_list) {
		...
		for (int i = 0; i < size; i++) {
			...
			String mheStr = "[ " + mhe.row + "  " + mhe.col + "  "
					+ mhe.workingHours + "  " + mhe.billType + "  "
					+ mhe.tseStatus + "  " + mhe.myRefTE + "  " + mhe.myRefMHE;
	...
	private String printMhinfo(ManHourInfo mhinfo) {
		...
		String edtStr = "Editable: "
				+ ((mhinfo.isManHourEditable) ? "YES" : "NO") + "\n\n";
		strBuf.append(edtStr);

		String hourlyUser = "Hourly Based User: "
				+ ((mhinfo.isHourlyBasedUser) ? "YES" : "NO") + "\n\n";
		strBuf.append(hourlyUser);

		String billInfo = "Internal Bill Info: \n" + "   [N] - "
				+ mhinfo.theBillTypeSet[0].billTypeInternal + "  "
				+ mhinfo.theBillTypeSet[0].billRateName + "   [O] - "
				+ mhinfo.theBillTypeSet[1].billTypeInternal + "  "
				+ mhinfo.theBillTypeSet[1].billRateName + "   [H] - "
				+ mhinfo.theBillTypeSet[2].billTypeInternal + "  "
				+ mhinfo.theBillTypeSet[2].billRateName + "   [W] - "
				+ mhinfo.theBillTypeSet[3].billTypeInternal + "  "
				+ mhinfo.theBillTypeSet[3].billRateName + "\n\n";
	...
	private ManHourEntry createManHourEntry(String projectName,
			String taskTypeDval, String billType, String bt_internal,
			String br_ref, String workingHours, String status, String tseref,
			String mheref, int row, int col) {
		...
		hourEntry.workRequired = mhInfo_.theMonthTmp[col].isWorkRequired ? "TRUE"
				: "FALSE";
	...
	private void bldPrgAndTskTypeMap() {
		pMap_ = null;
		if (mhInfo_ == null || mhInfo_.theProgramSet == null
	...
	private void bldBillTypeAndIvalMap() {
		btMap_ = null;
		if (mhInfo_ == null || mhInfo_.theBillTypeSet == null
				|| mhInfo_.theBillTypeSet.length == 0)
			return;
	...
	private void doInitMHEData(ManHourEntry[] mhe_set) {
		int mhCount = (mhe_set == null && mhe_set.length == 0) ? 0
				: mhe_set.length;
		...
		if (mhCount > 0) {
			...
			for (int mhIdx = 0; mhIdx < mhCount; mhIdx++) {
				...
				if (rowObjSet == null) {
					rowObjSet = new ManHourObject[table_column_names.length
							+ mhInfo_.totalDays];
				...
				if (oldMheObj != null) {
					ManHourObject mergedMheObj = ManHourObject.mergeMHEObject(
							oldMheObj, mhObj);
	...
	private void initManHourData() {
		try {
			ManHourEntry[] mhEntrySet = WorkmanagerOperation.load(session,
					mhInfo_);
	...
	private int invalidNormalHrsDay(String n_value, boolean over_time) {
		double n_value_i = (n_value != null && n_value.length() != 0) ? Double
				.parseDouble(n_value) : 0.0;
	...
	private int invalidTotalHrsDay(String n_value, String o_value,
			String w_value, String h_value) {
		double n_value_i = (n_value != null && n_value.length() != 0) ? Double
				.parseDouble(n_value) : 0.0;
		double o_value_i = (o_value != null && o_value.length() != 0) ? Double
				.parseDouble(o_value) : 0.0;
		double w_value_i = (w_value != null && w_value.length() != 0) ? Double
				.parseDouble(w_value) : 0.0;
		double h_value_i = (h_value != null && h_value.length() != 0) ? Double
				.parseDouble(h_value) : 0.0;
	...
	private boolean canBeAssigned(String n_value, String o_value,
			String w_value, String h_value) {
		Object[][] objData = wTable.getData();
		...
		try {
			...
			if (!checkProjectStartAndEndDay()) {
				...
				JOptionPane.showConfirmDialog(null, message1_1, "Warning...",
						JOptionPane.DEFAULT_OPTION);
				return canAssign;
			}
			...
			if (findNonEditingCell()) {
				...
				JOptionPane.showConfirmDialog(null, message1_2, "Warning...",
						JOptionPane.DEFAULT_OPTION);
				return canAssign;
			}
			...
			if (findEmptyProgramforAssign()) {
				...
				JOptionPane.showConfirmDialog(null, message1_3, "Warning...",
						JOptionPane.DEFAULT_OPTION);
				return canAssign;
			}
			...
			if (findEmptyTaskforAssign()) {
				...
				JOptionPane.showConfirmDialog(null, message1_4, "Warning...",
						JOptionPane.DEFAULT_OPTION);
				return canAssign;
			}
			...
			if (isProgramClosed()) {
				...
				JOptionPane.showConfirmDialog(null, message1_5, "Warning...",
						JOptionPane.DEFAULT_OPTION);
				return canAssign;
			}
			...
			if (choiceCnt > 1) {
				if (w_ok || h_ok) {
					...
					JOptionPane.showConfirmDialog(null, message3_1,
			...
			if (n_ok) {
				...
				if (whichday > 0) {
					String message5 = "Normal hours for day " + whichday
			...
			if (w_ok) {
				...
				if (whichday_w > 0) {
					...
					JOptionPane.showConfirmDialog(null, message6_0,
							"Warning...", JOptionPane.DEFAULT_OPTION);
					return canAssign;
				}
			}
			...
			if (h_ok) {
				...
				if (whichday_h > 0) {
					...
					JOptionPane.showConfirmDialog(null, message6_1,
							"Warning...", JOptionPane.DEFAULT_OPTION);
					return canAssign;
				}
			}

			int whichTotalHrsDay = invalidTotalHrsDay(n_value, o_value,
					w_value, h_value);
			if (whichTotalHrsDay > 0) {
				String message7 = "Total hours can not be greater than 24 !";
				JOptionPane.showConfirmDialog(null, message7, "Warning...",
						JOptionPane.DEFAULT_OPTION);
				return canAssign;
			}
			...
			if (n_ok) {
				...
				if (totalNormalHoursOfMonth > (double) totalNormalHoursAllowed_) {
					...
					JOptionPane.showConfirmDialog(null, message8, "Warning...",
							JOptionPane.DEFAULT_OPTION);
					return canAssign;
				}
			}
	...
	private void doCheckStatus() {
		...
		if (selectedRows.length != 1 && selectedColumns.length != 1) {
			...
			JOptionPane.showConfirmDialog(null, mes1, "Warning...",
					JOptionPane.DEFAULT_OPTION);
			return;
		}
		...
		if (obj0 == null || !ManHourObject.isMe(obj0)) {
			...
			JOptionPane.showConfirmDialog(null, mes2, "Warning...",
					JOptionPane.DEFAULT_OPTION);
			return;
		}

		List<ManHourObject> mheList = ManHourObject
				.getMheObjectList((ManHourObject) obj0);
		int size = (mheList != null) ? mheList.size() : 0;
		StringBuffer strBuf = new StringBuffer("\n");
		for (int pdx = 0; pdx < size; pdx++) {
			ManHourObject mheObj = mheList.get(pdx);

			String mes = "Day: " + (selectedColumns[0] + 1) + "   Work Hrs: "
					+ mheObj.getWorkingHours() + "   Status: "
					+ mheObj.getTseStatus() + "   TSE UID: "
					+ mheObj.getTseRef() + "   MHE UID: " + mheObj.getMheRef()
					+ "\n";
			strBuf.append(mes);
		}

		JOptionPane.showConfirmDialog(null, strBuf.toString(), "Status...",
				JOptionPane.DEFAULT_OPTION);
		return;
	}
		...
		for (int jdx = 0; jdx < colCnt; jdx++) {
			...
			for (int idx = 0; idx < rowCnt - 1; idx++) {
				...
				for (int pdx = 0; pdx < size; pdx++) {
					...
					String billType = mhObj.getBillType() == null ? "" : mhObj
							.getBillType();
					if (!billType.equals(ManHourObject.nBillType))
						continue;
	...
	private void doSumByBillType() {
		...
		if (totalNormalHoursAllowed_ != 0) {
			...
			double logging2 = b.setScale(2, BigDecimal.ROUND_HALF_UP)
					.doubleValue();
	...
	public ManHourProgram getManHourProgramByRow(int row) {
		...
		if (prgObj == null || prgObj.toString().length() <= 0 || ttObj == null
				|| ttObj.toString().length() <= 0) {
			return mhPrg;
		}
	...
	private void printObj() {
		...
		if (gObj == null) {
			...
			if (!ManHourObject.isMe(gObj)) {
				...
				System.out.println("obj: " + mgObj.toString() + "  billtype: "
						+ mgObj.getBillType() + "  status: "
						+ mgObj.getTseStatus() + "  ref: " + mgObj.getTseRef());
			}
		}
	}
}
