...
public class SWTutil {
	...
	private static final ResourceBundle reg = ResourceBundle
	...
	private HashMap<String, MyCostInfoBean> maps_CostInfo_Non;// 保存非人工的costInfo费用信息
	...
	// 构造函数
	public SWTutil(Shell shell, TCSession session,
				...
				if (flag) {
					...
					try {
						...
						TCComponentItemRevision saveAsRev = rev_tag
								.saveAs(item_version);
						//System.out.println("新版本是否有权限修改："+ saveAsRev.okToModify() + " 版本Name=========="+ saveAsRev);
						 ...
						 */
						COCOperation coc = new COCOperation(session, saveAsRev,
						...
						// openByPass();
						TCComponent tcComponents[] = saveAsRev
						...
						if (isF == 2 ) {
							...
							System.out.println("====checkExcel   end====");
							...
							if (error_list.size() != 0) {
								...
								TCComponentItem myProgramItem = saveAsRev.getItem();
						...
						if (tcComponents != null) {
							...
							for(TCComponent tcComponent:tcComponents){
								if(tcComponent.getType().equals("MSExcelX"))
						...
						if (isF == 2 ) {
							...
							// 删除无值的costInfo---费用信息 modify by wuwei--2019-06-13
						...
						openByPass();
						myProgramItem.lock();
						myProgramItem.setProperty("jci6_EQU", maps.get("EQU"));
						//myProgramItem.setProperties(itemPropertArray, values);
						myProgramItem.save();
						myProgramItem.unlock();
						closeByPass();
		...
		combo_version.addSelectionListener(new SelectionListener() {
			public void widgetSelected(SelectionEvent e) {
				...
				try {
					...
					//System.out.println("读取EXCEL中Signed Money==============="+ jci6_SignedMoney_str);
					...
					if (jci6_SignedMoney_str == null)
						...
						EQU_VALUE_str = "0";
					double doublejci6_SignedMoney = Double
							.parseDouble(jci6_SignedMoney_str);
					double rr = (double) Math
							.round(doublejci6_SignedMoney * 100) / 100;
					String jci6_SignedMoney = rr + "";

					//System.out.println("处理后的Signed Money==============="+ jci6_SignedMoney);
	...
	private void getAllValues(HashMap<String, String> hashmap) {
		...
		if (text_PDx_Sign_Date.getText() != null
			...
			//System.out.println("--text_PDx_Sign_Date----"+ text_PDx_Sign_Date.getText());
		...
		try {
			...
			if (!value.equals("")
				...
				if (maps.get("EQU") != null
					...
					//System.out.println("------jci6_EQU--------Finished!!!==========="+ maps.get("EQU"));
					...
					vector.add(tcProperties[1]);
					//System.out.println("------jci6_EQU==0------Finished!!!==========="+ maps.get("EQU"));
							
									
				}

				//System.out.println("------approved_amount****-------START=========");

						
				if (maps.get("approved_amount") != null
						&& Double.parseDouble(maps.get("approved_amount")) != 0) {
					//System.out.println("------jci6_SignedMoney---------"+ maps.get("approved_amount"));
							
					String str123 = maps.get("approved_amount");
					
					try{
						BigDecimal bd = new BigDecimal(str123);
						str123 = bd.stripTrailingZeros().toPlainString();
					}catch (Exception e) {
						// TODO: handle exception
						str123="0.00";
					}
					tcProperties[2].setDoubleValueData(Double.parseDouble(str123));
					vector.add(tcProperties[2]);

					//System.out.println("------jci6_SignedMoney-------lala===BigDecimal:: "+ str123);
					...
					vector.add(tcProperties[2]);
					//System.out.println("------jci6_SignedMoney==0------Finished!!!==========="+ maps.get("approved_amount"));
				...
				//System.out.println("----- set jci6_GEBTTemplate  successfully !!! ----");
						

				tcProperties[4].setStringValueData(maps.get("PDx_Squence"));
				vector.add(tcProperties[4]);
				//System.out.println("----- set jci6_PDxSeq  successfully !!! ----"+ maps.get("PDx_Squence"));
				...
				//System.out.println("After revise::PDx_Squence============="+ maps.get("PDx_Squence"));
				...
				//System.out.println("------jci6_PDxSignDate--set  Finished !!!--");
				...
				if (maps.get("approved_amount") != null
					...
					//System.out.println("------jci6_SignedMoney--------Finished!!!==========="+ maps.get("approved_amount"));
	...
	private String[] getLovStrings(TCComponentItem componentItem,
		...
		try {
			...
			System.out.println("componentItem:"+componentItem);
	...
	private String searchGEBTVersion_value(String lov_values,
		...
		//System.out.println("ww searchGEBTVersion_value--->" + value);
