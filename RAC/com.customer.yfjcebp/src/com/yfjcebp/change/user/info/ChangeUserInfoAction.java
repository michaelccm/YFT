 ...
 *
 */
public class ChangeUserInfoAction extends AbstractAIFAction {
	...
	public ChangeUserInfoAction(AbstractAIFUIApplication app, String arg1) {
		super(app, arg1);
	...
	private void getDisciplineMap(TCSession session) throws TCException {
		...
		TCComponent[] comps = JCI6YFJCUtil.query(textService, queryType,
				...
				new String[] { "*" });
		System.out.println("YFJC_Search_Discipline-->"+comps);
		
		if (comps != null) {
			System.out.println("YFJC_Search_Discipline  comps.length-->"+comps.length);
	...
	private void modifyPersonInfo(TCComponentPerson person,
		...
		for (int i = 0; i < person_pros.length; i++) {
			...
			//add 2014-12-4
			 ...
			 //法二
//			 try{
//					BigDecimal bd = new BigDecimal(s);  //1.0010064E7   1.238761976E-10
//					s = bd.toPlainString();
