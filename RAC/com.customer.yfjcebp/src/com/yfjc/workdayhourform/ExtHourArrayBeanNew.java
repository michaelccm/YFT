...
public class ExtHourArrayBeanNew extends AbstractPropertyBean{
	...
	public ExtHourArrayBeanNew(FormToolkit paramFormToolkit, Composite paramComposite, boolean paramBoolean, Map paramMap) {
		session = (TCSession) AIFUtility.getCurrentApplication().getSession();
		property = "";
		//System.out.println(" =======ExtHourArrayBeanNew======== " );
		mFrame=SWT_AWT.new_Frame(paramComposite);
	}
	...
	public boolean isPropertyModified(TCProperty arg0) throws Exception {
		...
		proTaskMap = myPanel.getProTaskMap();
		...
		if (row_cnt != old_size) {
			...
			for (int i = 0; i < row_cnt; i++) {
				...
				String name = "";
				if(proTaskMap != null){
					Map<String,TCComponentScheduleTask> taskMap = proTaskMap.get(prj);
					if(taskMap != null){
						...
						if (!name.equals(table.getValueAt(i, 2))) {
							...
							if(!taskMap.get(name).equals(task)){
								return true;
							}
						}
					}
	...
	public void load(TCProperty arg0) throws Exception {
		...
		try {
			...
			if (template == null) {
				...
				mFrame.add(myPanel, BorderLayout.CENTER);
			}
